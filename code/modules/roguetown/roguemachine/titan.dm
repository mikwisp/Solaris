	...
	if(findtext(message, "summon crown")) //This must never fail, thus place it before all other modestuffs.
		...
		if(SSroguemachine.crown)
			...
			if(I && !ismob(I.loc))//You MUST MUST MUST keep the Crown on a person to prevent it from being summoned (magical interference)
				...
				if(crown_area && istype(crown_area, /area/provincial/indoors/town/province_keep/vault) && notlord) //Anti throat snipe from vault
...
/obj/structure/roguemachine/titan/proc/give_tax_popup(mob/living/carbon/human/user)
	...
	if(newtax)
		...
		priority_announce("The new tax in Sunmarch shall be [newtax] percent.", "The Generous Lord Decrees", pick('sound/misc/royal_decree.ogg', 'sound/misc/royal_decree2.ogg'), "Captain")
...
/proc/make_outlaw(raw_message)
	if(raw_message in GLOB.outlawed_players)
		...
		priority_announce("[raw_message] is no longer an outlaw in the Sunmarch.", "The [SSticker.rulertype] Decrees", 'sound/misc/royal_decree.ogg', "Captain")
		return FALSE
	var/found = FALSE
